...
### 3.5 把同一个密码填进 SQL_DSN

编辑 `.env`，把 `SQL_DSN` 里的 `CHANGE_ME` 换成 3.4 生成的那串：

```
SQL_DSN=postgres://futureai_api:<3.4 的那串密码>@localhost:5432/futureai_api?sslmode=disable
```
...
`.env` 里的 `SQL_DSN` 密码和 `POSTGRES_PASSWORD` 不一致。
回 3.4 / 3.5 对一遍。
